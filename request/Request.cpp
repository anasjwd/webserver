...
// Connection::Connection(int fd) : sockfd(fd), headersParsed(false)
// {}
...
//     // For reverse find_if, we need a different approach
...
// Implement Connection constructor
Connection::Connection(int fd) : sockfd(fd), headersParsed(false) {}
...
// ... (keep all other methods the same until readFromSocket)
...
bool Request::readFromSocket(int socketfd)
{
    ...
    std::string buffer;
    char chunk[1024];
    bool headersParsed = false; // Initialize here
